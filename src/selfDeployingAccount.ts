 ...
 */
export abstract class SelfDeployingAccount
  extends Account
  implements SelfDeployingAccountInterface
{
  ...
  public async deployFrom(
    ...
    assert(
      isEqualAddress(deployedAddress, this.address),
      "The deployed address does not match the expected address",
    )
  ...
  protected async executeWithDeploy(
    calls: Call[],
    abis: Abi[] | undefined,
  ...
  async execute(
    ...
    const abis = Array.isArray(abisOrUniversalDetails)
      ? abisOrUniversalDetails
      : undefined
    universalDetails = Array.isArray(abisOrUniversalDetails)
      ? universalDetails
      : abisOrUniversalDetails
